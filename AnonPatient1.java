...
public class AnonPatient1 extends javax.swing.JFrame {
Connection con=null;
ResultSet rs=null;
...
private void Get_AnonymizedData() {
        String sql = "SELECT Age_Group, Gen, BG, SUBSTRING(Address, 1, 3) AS City, Disease_Category FROM ( SELECT FLOOR(Age / 5) * 5 AS Age_Group, Gen, BG, Address, CASE WHEN Remarks LIKE '%diarrhea%' THEN 'Digestive Disorder' WHEN Remarks LIKE '%headache%' THEN 'Headache Disorder' WHEN Remarks LIKE '%allergies%' THEN 'Allergic Disorder' WHEN Remarks LIKE '%concentrating%' THEN 'Cognitive Impairment' WHEN Remarks LIKE '%knee%' AND Remarks LIKE '%pain%' THEN 'Knee Pain' WHEN Remarks LIKE '%back%' AND Remarks LIKE '%pain%' THEN 'Back Pain' WHEN Remarks LIKE '%leg%' AND Remarks LIKE '%injury%' THEN 'Leg Injury' WHEN Remarks LIKE '%cold%' THEN 'Respiratory Illness' ELSE 'Other'  END AS Disease_Category FROM patientregistration) AS anonymized_data;";
        try {
